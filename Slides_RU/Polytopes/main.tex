...
\begin{frame}{Полупространства, Определение}
	\begin{flushleft}
		...
		Мы можем определить полупространство как множество всех точек $\mathbf{x}$, таких что $\mathbf{a}^\top \mathbf{x} \leq b$. Это имеет очень чёткую геометрическую интерпретацию. На следующем изображении закрашенное пространство \textbf{не} входит в полупространство.
...
\begin{frame}{Полупространство, проходящее через начало координат}
	\begin{flushleft}
...
\begin{frame}{Комбинация полупространств}
	\begin{flushleft}
		...
		Результирующая область будет легко описываться как $\begin{bmatrix} \mathbf{a}_1^\top \\ ... \\ \mathbf{a}_k^\top \end{bmatrix} \mathbf{x} \leq \begin{bmatrix} b_1 \\ ... \\ b_k \end{bmatrix}$
