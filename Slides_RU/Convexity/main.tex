...
\subtitle{Вычислительный интеллект, Лекция 4}
\author{Сергей Савин}
\centering
\date{\mydate}
...
\begin{frame}{Выпуклая комбинация, 2}
	\begin{flushleft}
		...
		\begin{equation}
			\mathcal{L} = \{ \bo{x}: \ \beta_1 \bo{x}_1 + \beta_2 \bo{x}_2, \ \beta_1+\beta_2 = 1, \ \beta_{1, 2} \geq 0 \}
		\end{equation}
...
\begin{frame}{Примеры выпуклых множеств, 1}
	\begin{flushleft}
		
		$\mathbf{x} \in \mathcal{X}$, $\mathcal{X} = \mathbb{R}^n$ - выпуклое множество.
		
		\bigskip
		
		$\mathbf{x} \in \mathcal{X}$, $\mathcal{X} = \{ \mathbf{x} \in \mathbb{R}^n: \ \mathbf{x} \leq \mathbf{h} \}$ - выпуклое множество.
		...
		$\mathbf{x} \in \mathcal{X}$, $\mathcal{X} = \{ \mathbf{x} \in \mathbb{R}^2: \ x_1^2+x_2^2 \leq h^2 \}$  - выпуклое множество.
...
\begin{frame}{Выпуклые функции}
	\begin{flushleft}
		...
		\input{figure_function_convexity}
