...
\subtitle{\mycoursetitle, Lecture 12}
...
x_{m+1}, ..., x_n \in \mathbb{Z}. &\ \ \ \ \textcolor{mylightgray}{\text{integer variables}}
\end{cases}
%
\end{aligned}
\end{equation}
...
\begin{itemize}
    ...
    \item The ``convex program" in the phrase ``mixed-integer convex program" should be understood as ``if we fix values of the integer variables, or relax them into reals, the program becomes convex".
    \item Mixed integer programs are usually solved using \emph{branch and bound algorithms}; in worst-case scenario, the algorithms performs exhaustive search.
...
\begin{frame}{Branch and bound}
	\begin{flushleft}
		...
		Branch and bound searches for the best solution by solving a series of convex optimization problems.
...
\begin{frame}{Example: cable robot, 1}
	\begin{flushleft}
		...
		Earlier we formulated the problem of finding the forces $\bo{f}_i  = \bo{p}_i \alpha_i$ that achive acceleration $\bo{a}$ as close to $\bo{a}^*$ as possible:
		%
		\begin{equation}
			\begin{aligned}
		...
		We can reformulate it, requiring the robot to \textcolor{blue}{choose only $K$ cables} to achieve this goal. To do that, we need to introduce binary variables $c_i$ indicating that the cable is switched on ($c_i = 1$) and the corresponding condition:
		%
		 \begin{align}
		 	| \alpha_i | \leq M c_i 
		 \end{align}
		 %
		 Since $\alpha_i \geq 0$, the condition simplifies to $\alpha_i \leq M c_i$. We further require that $\sum\limits_i c_i = K$.
...
\begin{frame}{Example: cable robot, 2}
	\begin{flushleft}
		...
		Then, the problem takes the following form:
		%
		\begin{equation}
			\begin{aligned}
