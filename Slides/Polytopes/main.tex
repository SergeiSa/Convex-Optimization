...
\begin{frame}{Convex polytopes, 1}
\begin{flushleft}
...
\begin{frame}{Convex polytopes, 2}
\begin{flushleft}

You can think of polytopes as geometric figures (continuous sets of points) with flat (linear) edges, faces, and their higher-dimensional analogues.

\bigskip

\begin{definition}
	A convex polytope is a bounded convex set that can be represented as the intersection of a finite number of half-spaces.
\end{definition}

\bigskip
 
 If a set is unbounded, we call it a \emph{polyhedron}.
 
\end{flushleft}
\end{frame}


\begin{frame}{Half-spaces, Definition}
\begin{flushleft}
...
\begin{frame}{Half-space passing through the origin}
\begin{flushleft}
...
\begin{frame}{Half-space, general case}
\begin{flushleft}
...
\begin{frame}{Combination of half-spaces}
\begin{flushleft}

We can define a convex set as an \emph{intersection} of half-spaces $\mathbf{a}_i^\top \mathbf{x} \leq b_i$:
...
And conversely, any matrix inequality \eqref{eq:ineq} represents either an empty set, a convex polytope (bounded) or a convex polyhedron (inbounded).

\bigskip

\begin{definition}
 $\mathbf{A} \mathbf{x} \leq  \mathbf{b}$ is called \emph{H-representation} (half-space representation) of a polyhedron.
\end{definition}
...
\begin{frame}{H-representation in COP}
	\begin{flushleft}
		...
		We can use H-polytope (polyhedron) constraints as a part of convex optimization problem. For example, the following QP includes such constraint:
...
\begin{frame}{Example}
	\begin{flushleft}
		...
		Find the robot position $\bo{r} = [x, \ y]\T$ in a room of length $l$ and width $w$, closest to the desired point $\bo{r}_d= [x_d, \ y_d]\T$; the coordinate system is aligned with axes of the room, the center of the room is the point (0, 0).
...
\begin{frame}{Convex hull}
	\begin{flushleft}
		...
		Given points $\bo{x}_1$, $\bo{x}_2$, ..., $\bo{x}_N$ their convex hull is given by:
		...
		Note that the constraints $\sum_{i=1}^{N} \alpha_i = 1$ and $\alpha_i \geq 0$ automatically restrict $\alpha_i$ to lie in the interval $[0, \ 1]$.
...
\begin{frame}{V-representation in COP}
	\begin{flushleft}
		...
		We can use V-polytope constraints as a part of a convex optimization problem. For example, the following QP includes such constraint:
...
Converting between H- and V-representations is generally computationally challenging.

\bigskip

\begin{itemize}
	\item H $\rightarrow$ V: vertex enumeration (finding all extreme points)
	\item V $\rightarrow$ H: facet enumeration (finding all defining inequalities)
\end{itemize}
...
\begin{frame}{Zonotopes: G-representation}
	\begin{flushleft}
		...
		A zonotope $\mathcal{Z}$ is a centrally symmetric convex polytope defined by its \emph{center} $\bo{c}$ and \emph{generator matrix} $\bo{G}$:
...
\begin{frame}{G-representation in COP}
	\begin{flushleft}
		...
		We can use zonotope constraint as a part of a convex optimization problem. For example, the following QP includes such constraint:
