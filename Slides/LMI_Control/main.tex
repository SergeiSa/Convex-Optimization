...
\subtitle{\mycoursetitle, Lecture 9}
...
\begin{frame}{S-procedure}
	\begin{flushleft}
		...
		There is a way to enforce constraint $\bo{z}^\top \bo{M}\bo{z} < 0$ for such $\bo{z}$ that $\bo{z}^\top \bo{N}\bo{z} > 0$. This is called \emph{S-procedure}.
		
		\begin{theorem}[S-procedure]
			If $\exists \gamma > 0$ such that $\bo{M} + \gamma \bo{N} \prec 0$ then $\bo{z}^\top \bo{N}\bo{z} > 0 \implies \bo{z}^\top \bo{M}\bo{z} < 0$ 
		\end{theorem}
		
		\bigskip
		
		Proof of sufficiency. Let us choose such $\bo{z}$ that $\bo{z}^\top \bo{N}\bo{z} > 0$. Then $0 > \bo{z}^\top (\bo{M} + \gamma \bo{N}) \bo{z} > \bo{z}^\top \bo{M}\bo{z}$. \textcolor{mygray}{(See more in the Appendix)}.
