...
\subtitle{\mycoursetitle, Lecture 5}
...
where $\mathbf{H}$ is positive-definite and $\mathbf{A}\mathbf{x} \leq \mathbf{b}$ describes a \emph{convex region}.
...
\begin{frame}{Bounded and unbounded domains, 1}
	\begin{flushleft}
		...
		The major difference between domains of the problems 2, 3 vs problems 1, 4 and 5 is that the latter are \emph{unbounded} (i.e., you can construct a sequence of the values in the domain that would approach infinity).
		
		\bigskip
		
		We can see that in the case of problems 3-5, bounded domain allows the problem to have a solution \textcolor{mygray}{(a continuous function over a compact set (closed + bounded) attains its minimum)}.
...
\begin{frame}{Bounded and unbounded domains, 2}
	\begin{flushleft}
		...
		This time, the fact that one of the \emph{boundaries} of the domain was not included (the domain is not \emph{closed}). This is why the problem 6 has no solution, while problem 7 has one. 
		
		\bigskip
		
		For the problem 6,  we can pick a value arbitrary close to $x = 2$, approaching it from the left, but for any such value, there always will be other values of the decision variable closer to $x = 2$ and hence producing larger values of $f$.
...
\begin{frame}{Convex combination, 1}
	\begin{flushleft}
		...
		A line segment between points $\bo{x}_1$ and $\bo{x}_2$ is a \emph{convex hull} of points $\bo{x}_1$ and $\bo{x}_2$. It can be defined as:
...
\begin{frame}{Examples of convex domains, 1}
	\begin{flushleft}
		...
		Proof: Let $\mathbf{x}_1, \mathbf{x}_2 \in \mathcal{X}$, so that 
		$\|\mathbf{x}_1\| \le h$ and $\|\mathbf{x}_2\| \le h$. Then for any $\alpha \in [0,1]$:
		\[  
		\|\alpha \mathbf{x}_1 + 
		\underset{\textcolor{mygray}{\text{triangle inequality}}}{(1-\alpha)\mathbf{x}_2\| \le \alpha \|\mathbf{x}_1\| }
		+ (1-\alpha)\|\mathbf{x}_2\|
...
\begin{frame}{Examples of convex domains, 2}
	\begin{flushleft}
		...
		For any positive-definite symmetric $\mathbf{H}$ it is true that $\mathbf{H} = \mathbf{D}^\top\mathbf{D}$, where $\mathbf{D} = \sqrt{\mathbf{H}}$ is called a matrix square root and it is full rank. 
		
		\bigskip
		
		With that $\mathbf{x}^\top \mathbf{H} \mathbf{x} \leq 1$ becomes $\mathbf{x}^\top \mathbf{D}^\top\mathbf{D} \mathbf{x} \leq 1$. Defining $\mathbf{y} = \mathbf{D} \mathbf{x}$ we get $\mathcal{X} = \{ \mathbf{D}^{-1}\mathbf{y}: \ \mathbf{y}^\top \mathbf{y} \leq 1 \}$. This is a linearly deformed ball, and as such it is also convex. \textbf{Linear (affine) transformations preserve convexity.}
...
\begin{frame}{Examples of non-convex domains}
	\begin{flushleft}
		...
		$\mathbf{x} \in \mathcal{X}$, $\mathcal{X} = \{ \mathbf{x} \in \mathbb{R}^n: \ \mathbf{x}^\top \mathbf{H} \mathbf{x} \geq 1 \}$, where $\mathbf{H} \succ 0$, is not convex. 
		
		\bigskip
		
		These proofs only require a single counter-example to show that the defining property of convex domains does not hold.
...
\begin{frame}{Supporting hyperplane}
	\begin{flushleft}
		...
		For a convex set $\mathcal C$ and a point $\bo{x}_0$ on its boundary, if inequality $\bo{a}\T \bo{x} \leq \bo{a}\T \bo{x}_0$ holds for $\forall \bo{x} \in \mathcal C$, then $\bo{a}\T \bo{x} = \bo{a}\T \bo{x}_0$ is a \emph{supporting hyperplane}.
...
\begin{frame}{Convex functions}
	\begin{flushleft}
		...
		A function is convex if, for any two points on its graph, the line segment connecting them lies above the graph.
...
\begin{frame}{Jensen’s inequality}
	\begin{flushleft}
		...
		This is a \emph{Jensen’s inequality}. It can be re-written equivalently as:
		%
		\begin{align}
			\begin{cases}
...
\begin{frame}{Appendix A. Types of domains}
	\begin{itemize}
		\item \textbf{Bounded}: variables cannot grow arbitrarily large
		\item \textbf{Unbounded}: variables can go to infinity
		\item \textbf{Closed}: contains its boundary
		\item \textbf{Open}: does not contain its boundary
