...
\begin{frame}{Example - drone trajectory, 3}
	\begin{flushleft}
		...
		We know that gravity in the world frame is expressed as $\bo{f}_g = mg \bo{e}_3$, where $\bo{e}_3 = [0 \ 0 \ 1 ]\T$ and $g \approx 9.81$.
		...
		\end{align}
		%
		With that, we can compute $\bo{b}_3$:
		%
		\begin{align}
...
\begin{frame}{Example - drone trajectory, 8 (extra)}
	\begin{flushleft}
		...
		\begin{enumerate}
			...
			\item Compute $\dot{ \bo{b}}_3 =  -\frac{1}{||\bo{f}_t||}(\bo{I} - \bo{b}_3 \bo{b}_3\T) \bo{j}$. \textcolor{mygray}{ We use the fact that $\bo{b}_3 = -\frac{\bo{f}_t}{||\bo{f}_t||}$ is a unit vector.}
