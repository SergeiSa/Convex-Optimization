...
\title{Linear and Quadratic programming}
...
where $\bo{H}$ is positive-semidefinite.
...
\begin{frame}{Slack variables}
	\begin{flushleft}
		...
		We can replace inequality constraints $\bo{A}\bo{x} \leq \bo{b}$ with equality constraints, by introducing positive slack varibles:
		%
		\begin{align}
			\begin{cases}
				\bo{A}\bo{x} - \bo{b} =  -\bo{s} \\
		...
		\end{align}
		%
		With that, we get a QP with two groups of variables: $\bo{x}$ constrained only by equalities and slack $\bo{s}$ constrained to lie in the positive cone:
		%
		\begin{equation}
			\begin{aligned}
...
\begin{frame}{Degenerate QP}
	\begin{flushleft}
		...
		Let us consider a QP with degenerate (not full rank) matrix $\bo{H} = \bo{R} \Sigma \bo{R}\T$ (without a loss of generality $\bo{H}$ can be assumed to be symmetric), where $\bo{R} \in \R^{n \times k}$.
		%
		\begin{equation}
			\begin{aligned}
				& \underset{\bo{x}}{\text{minimize}}
